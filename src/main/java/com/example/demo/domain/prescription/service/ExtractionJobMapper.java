...
public class ExtractionJobMapper {
     ...
     */
    private PrescriptionExtractionResult readResult(PrescriptionExtraction extraction) {
        String resultJson = extraction.getResultJson();
        if (resultJson == null) {
            return null;
        }
        try {
            return objectMapper.readValue(resultJson, PrescriptionExtractionResult.class);
